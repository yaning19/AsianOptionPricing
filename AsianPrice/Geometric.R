...
asianGeometricPricingMC<-function(S0,K,r,T,sigma,delta,N,call,AveragePrice){
  ...
  if (AveragePrice==1){
    ...
    for (j in 1:10000){      ##10000 MC SIMULATIONS
      ...
      if (N>1){
        Ave_St<-geometricmeanCol(St[c(2:(N+1)),])  ##IF N>1, CALCULATE GEOMETRIC MEAN 
                                                   ##OF STOCK PRICE
      }
  ...
  if (AveragePrice==0){
    ...
    for (j in 1:1000){
      ...
      if (N>1){
        Ave_St<-geometricmeanCol(St[c(2:(N+1)),]) 
      }
